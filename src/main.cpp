...
#define str(s) #s
#define xstr(s) str(s)
#define VSHADER_FP (xstr(PROJECT_SOURCE_DIR) "/src/vshader.glsl")
#define FSHADER_FP (xstr(PROJECT_SOURCE_DIR) "/src/fshader.glsl")
...
int main()
{
	...
	const gainput::DeviceId mouse_id = 
		input_manager.CreateDevice<gainput::InputDeviceMouse>();

	gainput::InputMap input_map(input_manager);
	input_map.MapBool(ButtonConfirm, mouse_id, gainput::MouseButtonLeft);
	...
	GLuint program_id = load_shaders(VSHADER_FP, FSHADER_FP);
	...
	do {
		...
		if (input_map.GetBoolWasDown(ButtonConfirm))
		{
			break;
